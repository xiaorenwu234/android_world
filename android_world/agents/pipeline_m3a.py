...
SUMMARY_PROMPT_TEMPLATE = (
    ...
    'UI elements before:\n{before_elements}\n'
    'UI elements after:\n{after_elements}\n\n'
    'Judge the OUTCOME of this step. IMPORTANT: a successful action does'
    ' NOT necessarily produce a visible change. Examples of "succeeded'
    ' with no/little visible change":\n'
    '  * Tapping an already-selected option / already-checked toggle.\n'
    '  * Drawing a thin stroke on a canvas (may be invisible at this'
    ' resolution).\n'
    '  * Drag-and-drop landing on the same/equivalent slot.\n'
    '  * "Save"/"Confirm" actions where any toast has already faded.\n'
    '  * `wait` / `keyboard_enter` / `navigate_back` style actions that'
    ' intentionally produce little visible diff.\n'
    '  * SOM labeling differences caused by accessibility-tree node'
    ' merging — even when the actual screen state DID change.\n\n'
    'Pick `failed` ONLY when there is CLEAR visual evidence the action'
    ' did NOT happen (e.g. the dialog is still open with the SAME unfilled'
    ' input, the same button still highlighted as untapped, an explicit'
    ' error/toast saying it failed). When in doubt, prefer'
    ' `inconclusive` rather than `failed` — downstream reasoning will'
    ' treat `inconclusive` as "probably worked, keep going".\n\n'
